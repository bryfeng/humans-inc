...
import { ContentListEditor } from '@/features/blocks/components';
import {
  createAndSaveBlock,
  createAndPublishBlock,
} from '@/features/blocks/actions';
import type { ContentListBlockContent } from '@/features/blocks/types';

export default function CreateContentListBlockPage() {
  const router = useRouter();
  const [title, setTitle] = useState('');
  const [content, setContent] = useState<ContentListBlockContent>({
    items: [
      {
        title: '',
        url: '',
  ...
  const handleSave = async (publish: boolean = false) => {
    ...
    const hasValidItem = content.items.some((item) => item.title.trim());
    if (!hasValidItem) {
      setError('Please add at least one item with a title before saving.');
      return;
    }
    ...
    try {
      const blockData = {
        user_id: user.id,
        position: 0,
        block_type: 'content_list' as const,
        title: title || 'Content List Block',
        content,
        config: {},
      };
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-semibold">
                  Create Content List Block
                </h1>
                <p className="text-foreground/60 text-sm">
                  Curate and share your favorite content
                </p>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Give your content list a title..."
          ...
          {/* Content List Editor */}
          <div>
            <ContentListEditor content={content} onChange={setContent} />
          </div>
        </div>
      </div>
    </div>
